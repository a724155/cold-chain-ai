 ...
 * 冷运 Agent 应用服务实现。
 *
 * <p>该类承担一次 Agent 问答用例的业务编排职责，
 * 不直接操作 ChatClient，也不关心具体模型厂商。</p>
 */
@Slf4j
@Service
...
public class ColdChainAgentApplicationServiceImpl implements IColdChainAgentApplicationService {
    ...
    public AgentAnswerDTO chat(AgentChatCommand command) {
        ...
        log.info("正式Agent问答开始，requestId={}，questionLength={}", requestId, command.getQuestion().length());

        try {
            // 调用Agent执行器，Application层不直接操作Spring AI ChatClient。
            String answer = agentExecutor.execute(requestId, command.getQuestion());
            ...
            return AgentAnswerDTO.of(requestId, answer, costMillis);
